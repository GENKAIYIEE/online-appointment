  ...
  const handleMarkAsRead = async (id?: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Dispatch global event for navbar
  ...
  const handleDelete = async (id?: string) => {
    try {
      ...
      if (res.ok) {
        ...
        window.dispatchEvent(
          new CustomEvent("notificationsUpdated", {
            detail: { action: id ? "markOneRevert" : "markAll" }, // Hack to trigger recount
          })
        );
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 font-heading">Admin Inbox</h1>
          <p className="text-sm text-slate-500 mt-1">
            Manage all real-time system notifications and alerts.
          </p>
        </div>
